...
class MyTab {
    ...
    fun tabs(tabpane: TabPane, text: String? = null, pathFile: File? = null) {
        if (pathFile != null) {
                            ...
                            form {
                                fieldset(labelPosition = Orientation.VERTICAL) {
                                    web = webview {
                                        ...
                                        println(url)
                                        engine.load(url)
                                        val win = engine.executeScript("window") as JSObject
                                        win.setMember("app", this@MyTab)
                                    }
    ...
    fun loadBody() {
        ...
        request.body = request.body.replace("\"", "\\\"")
        request.body = request.body.replace(System.getProperty("line.separator"), "\\n")
        request.body = request.body.replace("\n", "\\n")
        request.body = request.body.replace("\r", "\\n")
        web?.engine?.executeScript("test2('${request.body}')")
    }
}
